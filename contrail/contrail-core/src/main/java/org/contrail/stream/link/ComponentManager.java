...
public class ComponentManager {
	...
	public static final <U, D> ComponentLink<U, D> connect(Component source, Component destination) throws ComponentConnectionRejectedException {
		if (source instanceof SourceComponent && destination instanceof DestinationComponent) {
			...
			throw new ComponentConnectionRejectedException(MessagesProvider.from(ComponentManager.class).get("org/contrail/stream/message", "not.a.source").format());
		} else if (source instanceof SourceComponent) {
			throw new ComponentConnectionRejectedException(MessagesProvider.from(ComponentManager.class).get("org/contrail/stream/message", "not.a.destination").format());
		} else {
			throw new ComponentConnectionRejectedException(MessagesProvider.from(ComponentManager.class).get("org/contrail/stream/message", "not.a.source.and.destination").format());
		}
	}
