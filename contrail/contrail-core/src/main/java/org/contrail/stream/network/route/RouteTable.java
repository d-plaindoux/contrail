...
public class RouteTable {
	...
	public void addEntry(String name, ClientBuilder endpoint) throws RouteAlreadyExistException {
		if (this.hasEntry(name)) {
			throw new RouteAlreadyExistException(MessagesProvider.from(this).get("org/contrail/stream/message", "route.already.exist").format(name, endpoint));
		} else {
	...
	public ClientBuilder getEntry(String name) throws RouteNotFoundException {
		if (this.hasEntry(name)) {
			...
			throw new RouteNotFoundException(MessagesProvider.from(this).get("org/contrail/stream/message", "route.not.found").format(name));
		}
	}
