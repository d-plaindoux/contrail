...
public class EcosystemImpl implements Ecosystem {
	 ...
	 */
	private final Map<RegisteredUnitEcosystemKey, UpStreamDataHandlerFactory<?, ?>> hooks;
	...
	{
		this.hooks = new HashMap<RegisteredUnitEcosystemKey, UpStreamDataHandlerFactory<?, ?>>();
	 ...
	 */
	public <U, D> boolean addBinder(RegisteredUnitEcosystemKey ecosystemKey, UpStreamDataHandlerFactory<U, D> factory) {
	...
	public <U, D> UpStreamDataHandlerFactory<U, D> getBinder(final UnitEcosystemKey filter) throws CannotProvideComponentException {
		for (Entry<RegisteredUnitEcosystemKey, UpStreamDataHandlerFactory<?, ?>> unit : hooks.entrySet()) {
			if (filter.filteredBy(unit.getKey())) {
				return (UpStreamDataHandlerFactory<U, D>) unit.getValue();
			}
		}
