 ...
 * libraries like files. The connection requires to files. One for the input and
 * one for the output. Both are optional.
 * 
...
public class FileClient implements Closeable {
	 ...
	 */
	public FileClient(DataSenderFactory<byte[], byte[]> factory) {
		super();
	 ...
	 */
	public void connect(File input, File output) throws IOException, CannotCreateDataSenderException {
		assert !(input == null && output == null);

		final DataReceiver<byte[]> dataReceiver;

		if (output != null) {
			final FileOutputStream outputStream = new FileOutputStream(output);
			dataReceiver = new DataReceiver<byte[]>() {
				@Override
