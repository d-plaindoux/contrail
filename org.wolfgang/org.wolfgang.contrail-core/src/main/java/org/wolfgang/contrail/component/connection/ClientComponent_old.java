...
public class ClientComponent_old extends AbstractPipelineComponent<byte[], byte[], byte[], byte[]> {
	 ...
	 */
	@ContrailConstructor
	public ClientComponent_old(@ContrailArgument("context") ContextFactory contextFactory, @ContrailArgument("uri") String reference) throws URISyntaxException, ClientNotFoundException {
		super();
	...
	public ComponentLink connectDestination(DestinationComponentLink<byte[], byte[]> handler) throws ComponentConnectionRejectedException {
		return super.connectDestination(handler);
