...
/**
 * A Scanner provides a visitor of token stream. A token stream was in fact a
 * finite sequence of terms reflecting standardized lexical unit like string,
 * identified sequence of letters, integers, sequence of characters (without
 * letter and digit) and special for unrecognized characters.
 */

public class LexemeTokenizerImpl implements LexemeTokenizer {

	public static LexemeTokenizer create(InputStream input) throws ScannerException {
		final GenLex genLex = new GenLex(input);
		return new LexemeTokenizerImpl(genLex.getLexer());
	}
	 ...
	 */
	public LexemeTokenizerImpl(GenericLexer lexer) throws ScannerException {
		this.lexer = lexer;
	}
